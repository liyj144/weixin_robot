...
var moli_rb = function(robot_config, res, msg){
	if(!msg || !msg.Content){
		res.reply("给我点提示嘛，亲...");
	}
	var str = msg.Content;
	...
	var data = {"question": str, "api_key": rebot_config.app_id, "api_secret": rebot_config.app_secret};
	request.get({
        ...
        if("详情" == msg || "group" == msg || "g" == msg){
            db.collection("corp").aggregate([{$group: {_id: "$province",total:{$sum:1}}},{$sort:{total: -1}}], function(err, data){
                ...
                data.forEach(function(item, index){
                    message += "序号: " + (index + 1) +" ,省份: " + item._id + ", 数量: " + item.total + "<br/>";
                })
                res.reply(message);
            });
            ...
            db.collection("corp").find({corp_name:reg},{corp_name:1,province:1,_id:0}).limit(5).toArray(function(err, data){
                var message = "";
                data.forEach(function(item, index){
                    message += "序号: " + (index + 1) +" ,省份: " + item.province + ", 名称: " + item.corp_name + "<br/>";
                })
                res.reply(message);
            });
        }
